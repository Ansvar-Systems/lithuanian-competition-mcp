...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    switch (name) {
      ...
      case "lt_comp_get_decision": { const p = GetDecisionArgs.parse(args); const d = getDecision(p.case_number); if (!d) return errorContent(`Decision not found: ${p.case_number}`); const dr = d as unknown as Record<string, unknown>; return textContent({ ...d, _citation: buildCitation(String(dr.case_number || p.case_number), String(dr.title || dr.case_number || p.case_number), "lt_comp_get_decision", { case_number: p.case_number }, dr.source_url as string | undefined) }); }
